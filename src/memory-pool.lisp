...
(defun destroy-memory-pools (app)
  (let ((mp (memory-pool app)))
    ...
    (maphash #'(lambda (k v)
		 (declare (ignore k))
		 (when (typep v 'memory-resource-custom)
		   (let ((buffer (memory-resource-buffer v)))
		     (%vk:vkdestroybuffer (h (device buffer))
					  (h buffer)
					  (h (allocator buffer)))
		     (%vk:vkfreememory (h (device (allocated-memory buffer)))
				       (h (allocated-memory buffer))
				       (h (allocator (allocated-memory buffer)))))))
	     (slot-value mp 'allocated))
    (mapcar #'(lambda (mr)
		(let ((buffer (memory-resource-buffer mr)))
		  (%vk:vkdestroybuffer (h (device buffer))
				       (h buffer)
				       (h (allocator buffer)))
		  (%vk:vkfreememory (h (device (allocated-memory buffer)))
